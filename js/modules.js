  ...
  updateLogs: [
    {
      ...
      version: 'v3.3.54',
      content: `v3.3.54更新。
- 更新「设置 - 使用说明」中关于数据存储的描述：原先写“所有数据保存在 localStorage、不上传任何服务器”，已与实际架构不符
- 新说明：未登录时数据保存在本地浏览器 localStorage（离线缓存）；登录 GitHub 账号后自动同步到云端 Supabase PostgreSQL，支持多设备共享、断网回退本地、联网自动同步
- 新增「云端同步」条目：解释设置页 GitHub 登录开启同步、云端数据按账号隔离、业务数据（打卡/创作/素材/灵感/减肥记录）全量同步、AI 资讯与平台热点为只读缓存不同步
- 费用说明补充：云端同步使用 Supabase，个人免费额度内不收费；备份建议补充云端自动备份
- 缓存版本 ?v=85 → ?v=86`,
      impact: '使用说明准确反映「本地 + 云端同步」双存储架构，消除“数据存哪里”的误解',
    },
    {
      date: '2026-08-11',
      version: 'v3.3.53',
