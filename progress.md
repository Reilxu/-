...
### 2.3 部署与版本
- 当前版本：**v3.3.52**（糖罐替换为 PNG 图片；缓存版本 `?v=84`）。
- **镜像站**：`http://191.40.37.48` ✅ **v3.3.52**（已通过 `tar + expect auto-deploy.exp` 部署并验证 HTTP 200；curl 验证 `assets/candy-jar.png` 200（~1.04MB）、`js/habits.js?v=84` 含 `candy-jar.png`、`index.html?v=84` 含 `v3.3.52`）。
- **GitHub / Vercel**：`main` 已推送至 **v3.3.52**（commit `c96776e`）。GitHub 网络已恢复（curl `info/refs` 200），使用用户 PAT `ghp_Nx3...0Tok5O` 注入后 `push --no-thin` 成功（`bcc430f..c96776e`），PAT 已立即从 remote URL 清除（确认 `origin` 无 token）。WebFetch 跨网验证 Vercel 已上线 `js/habits.js?v=84` 与 `assets/candy-jar.png`；`index.html?v=84` 由 GitHub raw 验证含 `v3.3.52`。本地 `main` 与 `origin/main` 完全同步（0 ahead / 0 behind）。
...
- 当前版本 **v3.3.52**：移动端布局已修复、云端优先同步已落实；**镜像站 v3.3.52 + Vercel v3.3.52 均已部署** ✅。新增「减肥记录」模块（自律分组）：月份选择器 + 体重日历 + 今日体重卡（BMI/进度）+ AI 饮食/报告 + 记录琐事 + 趋势图 + 首启引导；v3.3.51 起对象型数据通过 `OBJECT_BUCKETS` 接入 Supabase 云端同步。v3.3.48 体验优化：运势常驻刷新按钮 + 今日页桌面间距放宽 + 减肥日历琐事查看面板与奶油底色对齐。v3.3.49 banner 加入透明背景打字猫 GIF。**v3.3.50：打卡模块图标整体由线条 SVG 改为 PNG（21 个，精灵图自动分割）、扭蛋机由 CSS 绘制改为单张 PNG 图**。**v3.3.52：糖罐（candy jar）也改为用户提供的卡通 PNG 图，糖果球仍叠在罐体玻璃区域**。
