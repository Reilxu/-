...
### 2.3 部署与版本
- 当前版本：**v3.3.53**（糖罐糖果改为罐体内部不规则堆积；缓存版本 `?v=85`）。
- **镜像站**：`http://191.40.37.48` ✅ **v3.3.53**（已通过 `tar + expect auto-deploy.exp` 部署并验证 HTTP 200；curl 验证 `index.html?v=85` 含 `v3.3.53`、`js/habits.js?v=85` 含 `JAR_TOP`/`HALF_CENTER`/`jarRand`、`assets/candy-jar.png` 200（~1.04MB））。
- **GitHub / Vercel**：`main` 当前为 **v3.3.52**（commit `c96776e`）。**v3.3.53 已本地 commit（`98a221c`）但未推送**——沙箱网络再次屏蔽 `github.com`（curl 探测 `info/refs` 返回 `000`、git push 报 `CONNECT tunnel failed, response 502`），本地 `main` 领先 `origin/main` **1 个提交**。PAT 用后已从 remote URL 清除（确认 `origin` 无 token）。需用户在本机执行 `git push origin HEAD:main`（仓库 `/Users/xuleng/WorkBuddy/ai/workspace`），或沙箱网络恢复后由我重新注入 PAT 推送。
...
- **v3.3.53 改动（糖罐糖果改为罐体内部不规则堆积）**：按用户反馈，把糖罐 PNG 里的糖果从「底部整齐一排」改为「玻璃肚内部不规则堆积 + 随罐体晃动」。①`js/habits.js` 的 `renderGlassJar` 不再把糖果用 flex 排成一行，而是为每颗糖生成绝对定位（`left%`/`top%`/`rotate`）。位置算法基于糖罐 PNG 视觉轮廓：罐体顶部约 34%、底部约 76%，用抛物线拟合罐肚宽度（颈部窄 13% 半宽 → 中部最宽 35% 半宽 → 底部 25% 半宽），y 方向用 `Math.pow(rand, 0.55)` 偏向底部模拟重力堆积，x 方向在对应 y 的罐体宽度内随机并留 7% 边距；每颗糖还有基于索引的确定性伪随机旋转角度（±35°），保证同一颗糖位置稳定复现。②`neo-brutalism.css` 第 17 节更新：`.jar-shake-wrap` 设为 `position:relative`；`.jar-candy-pile` 改为 `absolute` 并铺满 `100%×100%`；`.jar-candy` 改为 `absolute` 定位，保留圆角与内阴影；`.jar-overflow` 移到罐体右下角（`right 24%`、`bottom 26%`）并加白色半透明底。③晃动逻辑保持不变：`.jar-shaking` 仍会触发 `.jar-shake-wrap` / `.jar-candy-pile` / `.jar-candy` 的物理摆动动画，糖果会随 PNG 糖罐一起晃动。④`ICON_ASSET_VERSION` 84→85；`index.html` 全部 `?v=84`→`?v=85`（13 处）、版本串 `v3.3.52`→`v3.3.53`；`modules.js` 新增 v3.3.53 更新日志。**验证**：`node --check js/habits.js` / `js/modules.js` 通过、CSS 花括号 194=194 配对；镜像站 curl 验证 `index.html?v=85` 含 `v3.3.53`、`js/habits.js?v=85` 含 `JAR_TOP`/`HALF_CENTER`/`jarRand`、`assets/candy-jar.png` 200（~1.04MB）。
- **v3.3.52 改动（糖罐替换为 PNG 图片）**：按用户要求，把打卡模块「糖罐」子页的玻璃罐由 CSS/SVG 绘制改为用户提供的卡通 PNG。①用户图 `糖罐.png`（1254×1254 RGBA，透明底）复制到 `assets/candy-jar.png`（~1.04MB）。②`js/habits.js` 的 `renderGlassJar` 删除原 SVG 绘制（`jarGlassGrad`/`jarClip`/`jar-body` path 等），改为 `<img class="jar-png" src="assets/candy-jar.png?v=' + ICON_ASSET_VERSION + '">`；糖果球 `.jar-candy-pile` 与溢出标签 `.jar-overflow` 保留，仍用原有 `<div>` 渲染逻辑。③`neo-brutalism.css` 新增第 17 节：`.jar-glass` 去圆角/overflow、max-width 360px；`.jar-png` 宽度 100%；`.jar-candy-pile` 用百分比绝对定位（`left/right 20%`、`bottom 13%`、`top 40%`）覆盖在糖罐图片的玻璃肚区域；`.jar-overflow` 调整到玻璃右下角（`right 18%`、`bottom 11%`）。④`ICON_ASSET_VERSION` 82→84；`index.html` 全部 `?v=83`→`?v=84`（13 处）、版本串 `v3.3.51`→`v3.3.52`；`modules.js` 新增 v3.3.52 更新日志。**验证**：`node --check js/habits.js` / `js/modules.js` 通过、CSS 花括号 192=192 配对；镜像站 curl 验证 `assets/candy-jar.png` 200（1.04MB）、`js/habits.js?v=84` 含 `candy-jar.png`、`index.html?v=84` 含 `v3.3.52`；GitHub push 成功（`c96776e`）且 WebFetch 跨网验证 Vercel 已上线 `js/habits.js?v=84` 与 `assets/candy-jar.png`。
...
| 文件路径 | 说明 |
| --- | --- |
| `/Users/xuleng/WorkBuddy/ai/workspace/index.html` | 入口；加载 `style.css` → `neo-brutalism.css` → `weightloss.css?v=84`；版本字符串 `v3.3.52`；脚本统一 `?v=84` |
| `/Users/xuleng/WorkBuddy/ai/workspace/css/style.css` | **原站样式，换肤期禁止修改** |
| `/Users/xuleng/WorkBuddy/ai/workspace/css/neo-brutalism.css` | **主题覆盖层（当前 v84）**，所有 Neo-brutalism 视觉与修复补丁在此；第 15 节=图标 PNG 尺寸、第 16 节=扭蛋机 PNG、第 17 节=糖罐 PNG |
...
| `/Users/xuleng/WorkBuddy/ai/workspace/js/habits.js` | 习惯打卡模块；v3.3.50 `HABIT_ICONS` 改为 PNG `<img>`、扭蛋机改为单张 PNG；v3.3.52 糖罐改为单张 PNG；`ICON_ASSET_VERSION='84'` |
...
| `/Users/xuleng/WorkBuddy/ai/workspace/assets/candy-jar.png` | **v3.3.52 新增**：糖罐 PNG 图（复制自用户 `糖罐.png` 1254×1254），替代原 SVG 绘制玻璃罐 |
...
- 当前版本 **v3.3.53**：移动端布局已修复、云端优先同步已落实；**镜像站 v3.3.53 已部署** ✅，**Vercel 待推送**（当前仍为 v3.3.52，因沙箱网络再次屏蔽 GitHub）。新增「减肥记录」模块（自律分组）：月份选择器 + 体重日历 + 今日体重卡（BMI/进度）+ AI 饮食/报告 + 记录琐事 + 趋势图 + 首启引导；v3.3.51 起对象型数据通过 `OBJECT_BUCKETS` 接入 Supabase 云端同步。v3.3.48 体验优化：运势常驻刷新按钮 + 今日页桌面间距放宽 + 减肥日历琐事查看面板与奶油底色对齐。v3.3.49 banner 加入透明背景打字猫 GIF。**v3.3.50：打卡模块图标整体由线条 SVG 改为 PNG（21 个，精灵图自动分割）、扭蛋机由 CSS 绘制改为单张 PNG 图**。**v3.3.52：糖罐（candy jar）也改为用户提供的卡通 PNG 图**。**v3.3.53：糖罐里的糖果改为罐体玻璃肚内部不规则堆积，并随罐体一起晃动**。
